...
<body class="bg-[#1f2937] antialiased min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-2xl flex flex-col">
        <div class="bg-white rounded-[1.5rem] shadow-2xl p-4 sm:p-6 w-full mb-6">
            ...
            <div class="mt-6 text-center">
                <p class="text-[#1f2937] text-base sm:text-xl font-bold tracking-tight px-2">Pastikan wajah anda menengok ke kanan</p>
            </div>
        </div>

        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 w-full">
            <a href="{{ route('pendaftaran.form') }}" class="w-full sm:w-auto border-2 border-white/40 text-white text-center font-bold py-3.5 sm:py-2.5 px-10 rounded-xl hover:bg-white/10 hover:border-white transition duration-300 text-sm sm:text-base flex items-center justify-center">Batal</a>
            <button id="btn-capture" class="w-full sm:w-auto bg-white text-[#1f2937] font-bold py-3.5 sm:py-2.5 px-10 rounded-xl shadow-xl hover:bg-gray-200 transition duration-300 text-sm sm:text-base flex items-center justify-center">Lanjut (Manual)</button>
        </div>
    </div>
    ...
    <script>
        ...
        video.addEventListener('play', () => {
            speakText("Silakan menengok sedikit ke kanan.");

            intervalId = setInterval(async () => {
                // Periksa status penguncian di awal
                ...
                if (detections.length > 0) {
                    ...
                    if (distToRight > (distToLeft * 2.0)) {
                        ...
                        statusText.innerText = "Tengok ke Kanan...";
                    }
                }
            }, 300);
        });
        ...
        btnCapture.addEventListener('click', () => {
            if(capturedPhotos.length > 0) alert("Proses otomatis sedang berjalan.");
        });
    </script>
</body>
</html>