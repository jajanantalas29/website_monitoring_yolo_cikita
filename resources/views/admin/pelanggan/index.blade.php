...
<body class="bg-white h-screen flex overflow-hidden">
    ...
    <aside id="sidebar" class="w-64 bg-[#1f2937] text-white flex-col flex-shrink-0 transition-transform duration-300 fixed md:relative z-50 h-full transform -translate-x-full md:translate-x-0 flex">
        ...
        <nav class="flex-1 px-4 space-y-2">
            <a href="{{ route('admin.pelanggan') }}" class="flex items-center space-x-3 px-4 py-3 bg-white text-[#1f2937] rounded-l-full font-bold shadow-md transform translate-x-2">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden w-full bg-white relative">
        <header class="h-20 flex items-center justify-between px-6 md:px-8 bg-white shrink-0">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-[#1f2937] tracking-tight">Pelanggan</h2>
            </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[500px] md:min-w-full">
                        <thead class="bg-white sticky top-0 z-10">
                            <tr>
                                ...
                                <th class="p-4 border-b-2 border-gray-300 font-bold text-lg md:text-xl text-[#1f2937]">Nomor Telepon</th>
                        ...
                        <tbody class="text-gray-600">
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                <td class="p-4 py-4 md:py-6 font-medium">{{ $p->nama_lengkap }}</td>
                                <td class="p-4 py-4 md:py-6">{{ $p->nomor_telepon }}</td>
                                ...
                                <td class="p-4 py-4 md:py-6">
                                    <div class="flex items-center justify-center space-x-3">
                                        <a href="{{ route('admin.pelanggan.edit', $p->id) }}" class="text-gray-600 hover:text-[#1f2937]" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </a>
                                        <form action="{{ route('admin.pelanggan.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="4" class="p-8 text-center text-gray-400">Belum ada data pelanggan.</td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
