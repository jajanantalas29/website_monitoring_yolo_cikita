...
<head>
    ...
    <title>Status Pelanggan - Sistem Deteksi</title>
...
<body class="bg-white h-screen flex overflow-hidden relative">
    ...
    <form action="{{ route('admin.status.scan') }}" method="POST" class="absolute -z-50 opacity-0 h-0 w-0 overflow-hidden">
        @csrf
        <input type="text" name="uid_kartu" id="rfid_input" autofocus autocomplete="off">
    </form>
    ...
    <aside id="sidebar" class="w-64 bg-[#1f2937] text-white flex-col flex-shrink-0 transition-transform duration-300 fixed md:relative z-50 h-full transform -translate-x-full md:translate-x-0 flex">
        ...
        <nav class="flex-1 px-4 space-y-2 mt-2">
            ...
            <a href="{{ route('admin.status') }}" class="flex items-center space-x-3 px-4 py-3 bg-white text-[#1f2937] rounded-l-full font-bold shadow-md transform translate-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <main class="flex-1 flex flex-col bg-white overflow-hidden relative">
        ...
        <header class="h-20 flex items-center justify-between px-6 md:px-8 bg-white flex-shrink-0">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-[#1f2937] tracking-tight">Diruangan</h2>
            </div>
        ...
        <div class="flex-1 px-4 md:px-8 pb-6 overflow-y-auto space-y-6">
            ...
            @php
                // Memisahkan data pelanggan berdasarkan status keberadaan
                $pelangganDalam = $pelanggan->where('status_ruangan', 'di_dalam');
                $pelangganLuar = $pelanggan->where('status_ruangan', '!=', 'di_dalam');
            @endphp

            <div class="border-2 border-gray-300 rounded-2xl p-1 bg-white h-auto min-h-[40%] flex flex-col overflow-hidden shadow-sm">
                <div class="w-full h-full overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[500px] md:min-w-full">
                        ...
                        <tbody class="text-gray-600 text-sm md:text-base">
                            @forelse($pelangganDalam as $p)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                <td class="p-4 py-4 md:py-5 font-medium">{{ $p->nama_lengkap }}</td>
                                <td class="p-4 py-4 md:py-5">{{ $p->nomor_telepon }}</td>
                                <td class="p-4 py-4 md:py-5 text-gray-500 text-xs md:text-sm italic">
                                ...
                                <td class="p-4 py-4 md:py-5 text-center">
                                    <div class="flex items-center justify-center space-x-3">
                                        <a href="{{ route('admin.pelanggan.show', $p->id) }}" class="text-gray-600 hover:text-black transition" title="Lihat Detail">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </a>
                                        <div class="h-3.5 w-3.5 rounded-full bg-green-500 shadow-sm border border-green-200" title="Di Ruangan"></div>
                                    </div>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="p-8 text-center text-gray-400 italic">Belum ada pelanggan di ruangan</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <h2 class="text-2xl md:text-3xl font-bold text-[#1f2937] tracking-tight pt-2">Tidak Diruangan</h2>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                ...
                                <td class="p-4 py-4 md:py-5 text-center">
                                    <div class="flex items-center justify-center space-x-3">
                                        ...
                                        <div class="h-3.5 w-3.5 rounded-full bg-red-500 shadow-sm border border-red-200" title="Tidak Di Ruangan"></div>
                                    </div>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="p-8 text-center text-gray-400 italic">Semua pelanggan sedang berada di ruangan</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        ...
        // SCRIPT BARU UNTUK RFID AUTO-FOCUS
