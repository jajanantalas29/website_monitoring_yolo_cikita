...
<head>
    ...
    <title>Daftar Kartu - Admin</title>
...
<body class="bg-white h-screen flex overflow-hidden">
    ...
    <aside id="sidebar" class="w-64 bg-[#1f2937] text-white flex-col flex-shrink-0 transition-transform duration-300 fixed md:relative z-50 h-full transform -translate-x-full md:translate-x-0 flex">
        <div class="h-24 flex flex-col items-center justify-center border-b border-gray-700 mb-6">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden w-full bg-white relative">
        <header class="h-20 flex items-center justify-between px-6 md:px-8 bg-white shrink-0">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-[#1f2937] tracking-tight">Daftar Kartu</h2>
            </div>
        ...
        <div class="flex-1 p-4 md:p-8 pt-2 overflow-y-auto">
            ...
            <div class="flex justify-end mb-4">
                <a href="{{ route('admin.kartu.create') }}" class="bg-[#1f2937] text-white font-medium px-5 py-2.5 rounded-lg shadow-sm hover:bg-slate-700 transition text-sm md:text-base">
                    Tambah Kartu
                </a>
            </div>

            <div class="border-2 border-gray-300 rounded-2xl h-full p-1 overflow-hidden">
                <div class="w-full h-full overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[500px] md:min-w-full">
                        <thead class="bg-white sticky top-0 z-10">
                            <tr>
                                <th class="p-4 border-b-2 border-gray-300 font-bold text-lg md:text-xl text-[#1f2937]">Nama</th>
                                <th class="p-4 border-b-2 border-gray-300 font-bold text-lg md:text-xl text-[#1f2937]">UID Kartu</th>
                                <th class="p-4 border-b-2 border-gray-300 font-bold text-lg md:text-xl text-[#1f2937]">Foto Wajah</th>
                                <th class="p-4 border-b-2 border-gray-300 font-bold text-lg md:text-xl text-[#1f2937] text-center">Lainnya</th>
                            </tr>
                        </thead>
                        <tbody class="text-gray-600">
                            @forelse($kartus as $k)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                <td class="p-4 py-4 md:py-6 font-medium">{{ $k->pelanggan->nama_lengkap ?? 'Pelanggan Dihapus' }}</td>
                                <td class="p-4 py-4 md:py-6">{{ $k->uid_kartu }}</td>
                                <td class="p-4 py-4 md:py-6 text-gray-500 text-xs md:text-sm italic">
                                    @if($k->pelanggan)
                                        {{ $k->pelanggan->foto_lurus ? 'lurus.jpg, ' : '' }}
                                        {{ $k->pelanggan->foto_kiri ? 'kiri.jpg, ' : '' }}
                                        {{ $k->pelanggan->foto_kanan ? 'kanan.jpg, ' : '' }}
                                        {{ $k->pelanggan->foto_mulut ? 'mulut.jpg' : '' }}
                                    @else
                                        Tidak ada foto
                                    @endif
                                </td>
                                <td class="p-4 py-4 md:py-6">
                                    <div class="flex items-center justify-center space-x-3">
                                        <a href="{{ route('admin.kartu.show', $k->id) }}" class="text-gray-600 hover:text-[#1f2937]" title="Lihat Detail">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </a>
                                        <form action="{{ route('admin.kartu.destroy', $k->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kartu ini?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 hover:text-red-700" title="Hapus Kartu">
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="p-8 text-center text-gray-400">Belum ada kartu yang didaftarkan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
